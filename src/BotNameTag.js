...
function BotNameTag({ bot }) {
    return !bot ? <div>...</div> : (
        <div className="container-3baos1">
            <div className="avatarWrapper-2yR4wp" aria-label="Set status" role="button" tabIndex={0}>
                <div className="avatar-SmRMf2 wrapper-3t9DeA" role="img" aria-hidden="false" style={{ width: '32px', height: '32px' }}>
                    <svg width={40} height={32} viewBox="0 0 40 32" className="mask-1l8v16 svg-2V3M55" aria-hidden="true">
                        <foreignObject x={0} y={0} width={32} height={32} mask="url(#svg-mask-avatar-status-round-32)">
                            <div className="avatarStack-2Dr8S9">
                                <img src={bot.user.avatarURL || bot.user.defaultAvatarURL} alt=" " className="avatar-VxgULZ" aria-hidden="true" />
                            </div>
                        </foreignObject>
            ...
            <div className="nameTag-3uD-yy canCopy-2VBT7N" aria-label="Click to copy username" role="button" tabIndex={0}>
                <div className="colorStandard-2KCXvj size14-e6ZScH usernameContainer-1fp4nu">
                    <div className="size14-e6ZScH title-eS5yk3">{bot.user.username}</div>
                </div>
                <div className="size12-3cLvbJ subtext-3CDbHg">#{bot.user.discriminator}</div>
            </div>
